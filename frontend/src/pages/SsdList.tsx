...
const SsdList = () => {
    ...
    const loadProducts = async () => {
        ...
        setLoading(true)
        try {
            const data = await fetchProductsByCategory(ProductCategory.SSD, page, pageSize, {
            ...
            setProducts(data)
            const total = await getTotalProductsByCategory(ProductCategory.SSD, {
                minPrice: minPrice ?? undefined,
                maxPrice: maxPrice ?? undefined,
                minSize: minSize ?? undefined,
                maxSize: maxSize ?? undefined,
            ...
            console.error('Error loading SSD products:', err)
        } finally {
            setLoading(false)
            ...
            <Grid container spacing={3}>
                <Grid size={{ xs: 12, md: 3 }}>
                    <Paper elevation={1} sx={{ p: 3, borderRadius: 3 }}>
                        ...
                        <TextField label="Protocol" fullWidth margin="dense" value={protocol} onChange={e => setProtocol(e.target.value)} />
                        <TextField label="Form Factor" fullWidth margin="dense" value={formFactor} onChange={e => setFormFactor(e.target.value)} />
